...
public class CourseServiceImpl extends BaseServiceImpl<CourseDTO, Long, Course, CourseConverter> {
	...
	public ResponseEntity<String> updateCourseById(Long courseId, CourseDTO requestCourseDTO, MultipartFile image)
	{
		try {
			...
			if (optCourse.isPresent()) {
				...
				course.setPrice(requestCourseDTO.getPrice());
				course.setFree(requestCourseDTO.getFree());
				course.setTeacherId(requestCourseDTO.getTeacherId());
				if (image != null) {
					if(!image.isEmpty()){
						if(course.getImagePath()!=null && !course.getImagePath().isEmpty())
						{
							fileStorageService.deleteFile(course.getImagePath());
							LOGGER.info("File related to course with id " + courseId + " was deleted");
						}
						String filename = fileStorageService.store(image, "content");
						LOGGER.info("File related to course with id " + courseId + " was stored");
						course.setImagePath(filename);
					}
				}
				...
				LOGGER.warn("Attempt to update course with id: " + courseId + " failed because course could not be found");
				return ResponseEntity
